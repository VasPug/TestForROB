// setup-repo-mapping.ts — Creates the Repo Mapping Notion database.
// Idempotent: if a database titled "Repo Mapping" already exists under the parent page, returns existing ID.
...
interface SetupRepoMappingInput {
    parent_page_id: string;
    notion_api_key: string;
}

interface SetupRepoMappingOutput {
    database_id: string;
    created: boolean;
    warnings: string[];
}

const DB_TITLE = 'Repo Mapping';
...
async function handle(event: any, context: any): Promise<SetupRepoMappingOutput> {
    const input: SetupRepoMappingInput = event.payload;
    ...
    context.log(`[setup-repo-mapping] Checking for existing database "${DB_TITLE}" under page ${parent_page_id}`);
    const existingId = await findExistingDatabase(context, notion_api_key, parent_page_id, DB_TITLE);

    if (existingId) {
        context.log(`[setup-repo-mapping] Database already exists: ${existingId}`);
        return { database_id: existingId, created: false, warnings: [] };
    }

    context.log(`[setup-repo-mapping] Creating database "${DB_TITLE}"`);
    ...
    context.log(`[setup-repo-mapping] Database created: ${databaseId}`);

    if (warnings.length > 0) {
        context.log(`[setup-repo-mapping] Warnings: ${warnings.join('; ')}`);
    }
